...
@dataclass
class Gremlins_Old(Mocap):  # pylint: disable=too-many-instance-attributes
    """Gremlins (moving objects we should avoid)"""

    name: str = 'old_gremlins'
    ...
    def cal_cost(self):
        ...
        for contact in self.engine.data.contact[: self.engine.data.ncon]:
            ...
            if any(n.startswith('old_gremlin') for n in geom_names) and any(
    ...
    def pos(self):
        ...
        # pylint: disable-next=no-member
        return [self.engine.data.body(f'old_gremlin{i}obj').xpos.copy() for i in range(self.num)]
